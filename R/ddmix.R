# ddmix <- function (dat, ...) UseMethod("ddmix")

# ddmix.emmix <- function (dat, model, ...) {

#     n <- nrow(dat)
#     p <- ncol(dat)

#     if (is.null(model$dof))
#         model$dof <- rep(1000, g)
    
#     if (is.null(model$delta))
#         model$delta <- array(0, c(p, g))
    
#     ndist <- switch(tolower(model$distr), mvn = 1, mvt = 2, msn = 3,
#         mst = 4, 5)
...
#     if (length(c(model$mu)) != (p * g))
#         stop(paste("mu should be a ", p, "by", g, "matrix!"))
    
#     if (length(c(model$sigma)) != (p * p * g))
#         stop(paste("sigma should be a ", p, "by", p, "by", g,
#             " array!"))
    
#     if (length(c(model$dof)) != g)
#         stop(paste("dof should be a ", g, " vector!"))

#     if (length(c(model$delta)) != (p * g))
#         stop(paste("delta should be a ", p, "by", g, " array!"))
    
#     obj <- .C("ddmix", PACKAGE = "EMMIX", as.double(dat), as.integer(n),
#         as.integer(p), as.integer(g), as.integer(ndist), as.double(model$mu),
#         as.double(model$sigma), as.double(model$dof), as.double(model$delta), 
